/* JStudyC Question Data — Auto-generated from PDF Bank Soal */
const QUESTION_DATA = {
  "blok-ii-3": {
    id: "blok-ii-3",
    name: "Blok II.3",
    fullName: "Seizure, Unconsciousness, and Sense Organ Problem",
    exams: {
      "cumex-1": {
        name: "Cumex 1",
        types: {
          "reguler": {
            name: "Reguler",
            years: {
              "2021": [
                        {
                                  "id": 1,
                                  "question": "Pasien laki-laki 45 tahun datang dengan keluhan demam dan penurunan kesadaran yang memberat sejak 1 minggu terakhir. Pada pemeriksaan CT scan kepala dengan pemberian kontras, dokter mencurigai adanya infeksi sistem saraf pusat fase late cerebritis . Apakah temuan radiologis yang paling mungkin terlihat pada fase tersebut?",
                                  "options": [
                                            "Ring enhancement dengan batas ireguler",
                                            "Kapsul kolagen tebal dengan penyengatan tegas",
                                            "Area hipodens difus tanpa penyengatan kontras",
                                            "Massa hiperdens yang menandakan perdarahan fokal",
                                            "Area kalsifikasi sentral pada ganglia basalis"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "Ring enhancement dengan batas ireguler Abses otak berkembang melalui 4 tahap: early cerebritis, late cerebritis, early capsule formation , dan late capsule formation . Pada fase late cerebritis (hari 4-13), pusat nekrotik mulai terbentuk dengan batas tepi ireguler yang menyengat (ring enhancement tipis/ireguler) pada CT scan dengan kontras. Kapsul tebal yang tegas (opsi B) baru terlihat pada tahap capsule formation ."
                        },
                        {
                                  "id": 2,
                                  "question": "Pasien perempuan 35 tahun datang ke poliklinik dengan keluhan pusing berputar hebat yang memberat setiap kali ia menoleh ke kanan. Keluhan ini disertai mual tanpa muntah. Pemeriksaan neurologis dan fungsi pendengaran dalam batas normal. Manakah tatalaksana farmakologis spesifik yang paling tepat untuk mengurangi keluhan pasien ini?",
                                  "options": [
                                            "Betahistine",
                                            "Paracetamol",
                                            "Amitriptyline",
                                            "Ibuprofen",
                                            "Ondansetron"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "Betahistine Pasien mengalami vertigo perifer (kemungkinan BPPV). Betahistine adalah obat analog histamin yang sering digunakan sebagai tatalaksana farmakologis spesifik untuk mengatasi vertigo karena dapat memperbaiki mikrosirkulasi di telinga dalam."
                        },
                        {
                                  "id": 3,
                                  "question": "Area korteks otak manusia terbagi menjadi beberapa regio fungsional, salah satunya adalah gyrus postcentralis pada lobus parietalis. Apabila seorang pasien mengalami lesi iskemik terbatas pada area tersebut, fungsi utama apakah yang paling mungkin mengalami gangguan secara langsung?",
                                  "options": [
                                            "Fungsi somatosensorik",
                                            "Fungsi visual primer",
                                            "Fungsi pendengaran",
                                            "Fungsi motorik volunter",
                                            "Fungsi olfaktori"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "Fungsi somatosensorik Korteks postcentralis yang terletak di lobus parietalis merupakan korteks somatosensorik primer. Area ini berfungsi memproses sensasi raba, nyeri, suhu, dan propriosepsi dari seluruh tubuh."
                        },
                        {
                                  "id": 4,
                                  "question": "Seorang pasien laki-laki 50 tahun dengan riwayat asma bronkial persisten berat dijadwalkan menjalani operasi laparotomi darurat. Dokter anestesi berencana melakukan induksi general anesthesia (GA). Manakah agen induksi intravena berikut yang paling direkomendasikan karena memiliki efek relaksasi otot polos bronkus dan bronkodilatasi?",
                                  "options": [
                                            "Etomidate",
                                            "Midazolam",
                                            "Ketamine",
                                            "Succinylcholine",
                                            "Rocuronium"
                                  ],
                                  "correctAnswer": 2,
                                  "explanation": "Ketamine Ketamine adalah agen induksi anestesi yang memiliki sifat bronkodilator kuat melalui pelepasan katekolamin endogen (efek simpatomimetik). Obat ini merupakan pilihan utama untuk induksi pasien dengan asma atau bronkospasme berat."
                        },
                        {
                                  "id": 5,
                                  "question": "Pasien laki-laki 60 tahun datang ke IGD dengan keluhan sesak napas, mual, dan bengkak pada kedua tungkai yang makin memberat sejak 1 bulan terakhir. Pemeriksaan fisik menunjukkan tekanan darah 140/95 mmHg. Pemeriksaan laboratorium menunjukkan kadar BUN 101 mg/dL, Kreatinin 9,2 mg/dL, dan GDS 121 mg/dL. Apakah diagnosis yang paling tepat pada pasien tersebut?",
                                  "options": [
                                            "Penyakit Ginjal Kronik (PGK)",
                                            "Sindrom Nefrotik",
                                            "Pielonefritis Akut",
                                            "Sindrom Nefritik",
                                            "Gagal Jantung Kongestif"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "Penyakit Ginjal Kronik (PGK) Peningkatan ekstrem pada ureum (BUN 101) dan kreatinin (9,2) mengindikasikan kerusakan ginjal yang sangat berat. Pada orang dewasa/lansia dengan nilai kreatinin setinggi ini dan disertai dengan sindrom uremia (mual, sesak, edema), diagnosis paling umum dan mengarah kuat adalah Penyakit Ginjal Kronik stadium akhir (ESRD), meski diagnosis pasti memerlukan riwayat penyakit selama minimal 3 bulan."
                        },
                        {
                                  "id": 6,
                                  "question": "Seorang korban kecelakaan lalu lintas dibawa ke IGD oleh petugas ambulans. Saat tiba, pasien tidak sadar, mengalami syok hemoragik akibat fraktur pelvis, dan terlihat mengalami distres pernapasan berat yang membutuhkan intubasi segera. Berdasarkan Emergency Severity Index (ESI), pada level berapakah triase pasien ini diklasifikasikan?",
                                  "options": [
                                            "Level 1",
                                            "Level 2",
                                            "Level 3",
                                            "Level 4",
                                            "Level 5"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "Level 1 ESI Level 1 diberikan pada pasien yang membutuhkan intervensi penyelamatan nyawa secara segera (immediate life-saving intervention). Pasien tidak sadar dengan syok hemoragik dan distres napas yang butuh intubasi mutlak masuk dalam Level 1."
                        },
                        {
                                  "id": 7,
                                  "question": "Dalam pedoman Advanced Life Support (ALS) terbaru, resusitasi pada pasien dewasa mengutamakan pendekatan C-A-B (Circulation-Airway-Breathing). Namun, pada protokol resusitasi neonatus dan anak, pendekatan sekuens A-B-C masih lebih sering ditekankan pada evaluasi awal. Apakah alasan klinis utama yang mendasari perbedaan tersebut?",
                                  "options": [
                                            "Penyebab utama henti jantung pada anak adalah hipoksia/asfiksia",
                                            "Tulang rusuk anak lebih kaku sehingga kompresi kurang efektif",
                                            "Kapasitas sisa fungsional (FRC) paru-paru anak jauh lebih besar",
                                            "Anak memiliki volume darah yang lebih banyak secara proporsional",
                                            "Anak lebih rentan mengalami henti jantung akibat fibrilasi ventrikel primer"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "Penyebab utama henti jantung pada anak adalah hipoksia/asfiksia Berbeda dengan dewasa yang umumnya mengalami henti jantung karena etiologi kardiak (VF/VT pulseless), sebagian besar kasus henti jantung pada bayi dan anak disebabkan oleh masalah pernapasan yang memburuk (asfiksia) yang berujung pada bradikardia dan asistol. Oleh karena itu, memastikan patensi jalan napas (A) dan ventilasi (B) sangatlah esensial."
                        },
                        {
                                  "id": 8,
                                  "question": "Seorang anak laki-laki berusia 3 tahun dibawa ke IGD karena kejang kelojotan seluruh tubuh sejak 10 menit yang lalu. Setibanya di IGD, pasien masih tidak sadar dan masih mengalami kejang tonik-klonik. Apakah golongan obat lini pertama yang paling tepat diberikan untuk menghentikan kondisi tersebut?",
                                  "options": [
                                            "Benzodiazepine",
                                            "Barbiturat",
                                            "Hidantoin",
                                            "Asam Valproat",
                                            "Karbamazepin"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "Benzodiazepine Pada tata laksana status epileptikus atau kejang akut pada anak, obat lini pertama yang direkomendasikan untuk menghentikan kejang segera adalah golongan benzodiazepine, seperti Diazepam (IV/rektal) atau Midazolam (IM/bukal/intranasal)."
                        },
                        {
                                  "id": 9,
                                  "question": "Pasien perempuan 55 tahun dengan riwayat neuropati diabetik mendapatkan resep obat pregabalin/gabapentin dari dokter spesialis saraf. Beberapa hari kemudian, pasien datang kembali mengeluhkan efek samping obat tersebut yang dirasa sangat mengganggu kesehariannya. Apakah efek samping yang paling umum terjadi pada penggunaan obat tersebut?",
                                  "options": [
                                            "Pusing ( dizziness ) dan mengantuk ( somnolence )",
                                            "Insomnia berat dan agitasi psikomotor",
                                            "Perdarahan saluran cerna bagian atas",
                                            "Hipoglikemia nokturnal persisten",
                                            "Retensi urin dan konstipasi obstipatif"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "Pusing ( dizziness ) dan mengantuk ( somnolence ) Pregabalin dan gabapentin adalah gabapentinoid yang bekerja menekan aktivitas sistem saraf pusat. Efek samping paling umum dari obat golongan ini yang sering dikeluhkan pasien adalah pusing, mengantuk, letargi, dan ataksia ringan, terutama pada awal penggunaan terapi."
                        },
                        {
                                  "id": 10,
                                  "question": "Pasien laki-laki 65 tahun dengan riwayat perdarahan lambung (tukak peptik) datang dengan keluhan nyeri sendi lutut hebat akibat osteoarthritis. Dokter merencanakan pemberian analgesik golongan NSAID yang memiliki profil keamanan gastrointestinal lebih baik dengan mekanisme kerja sebagai selective COX-2 inhibitor . Manakah obat di bawah ini yang termasuk dalam golongan tersebut?",
                                  "options": [
                                            "Celecoxib",
                                            "Ibuprofen",
                                            "Diclofenac",
                                            "Ketorolac",
                                            "Naproxen"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "Celecoxib Celecoxib adalah Obat Anti Inflamasi Non-Steroid (NSAID) yang bekerja menghambat enzim siklooksigenase-2 (COX-2) secara selektif. Obat ini diindikasikan untuk pasien yang membutuhkan NSAID namun berisiko tinggi mengalami komplikasi gastrointestinal (seperti pasien dengan riwayat tukak lambung). Ibuprofen, diclofenac, ketorolac, dan naproxen merupakan NSAID non-selektif."
                        }
              ],
              "2022": [
                        {
                                  "id": 1,
                                  "question": "Seorang laki-laki berusia 55 tahun datang ke IGD dengan keluhan dada berdebar dan pusing ringan sejak 1 jam yang lalu. Pemeriksaan tanda vital menunjukkan tekanan darah 110/70 mmHg dan nadi 58 x/menit ireguler. Pada pemeriksaan EKG didapatkan interval PR konstan, namun sesekali terdapat gambaran gelombang P yang tidak diikuti oleh kompleks QRS (dropped beat). Apakah diagnosis yang paling tepat pada pasien ini?",
                                  "options": [
                                            "AV blok derajat 1",
                                            "AV blok derajat 2 tipe 1",
                                            "AV blok derajat 2 tipe 2",
                                            "AV blok derajat 3",
                                            "Sinus pause"
                                  ],
                                  "correctAnswer": 2,
                                  "explanation": "(AV blok derajat 2 tipe 2) AV Blok derajat 2 tipe 2 (Mobitz II) ditandai dengan interval PR yang konstan, namun mendadak terdapat gelombang P yang tidak dilanjutkan oleh kompleks QRS ( dropped beat )."
                        },
                        {
                                  "id": 2,
                                  "question": "Seorang perempuan berusia 45 tahun dibawa keluarganya ke poliklinik saraf karena sering mengeluh nyeri kepala kronis progresif sejak 3 bulan terakhir disertai kejang fokal pada lengan kanan. Pada pemeriksaan MRI kepala dengan kontras, didapatkan massa ekstra- aksial berbatas tegas dengan gambaran dural tail sign di regio parasagital kiri. Apakah diagnosis radiologis yang paling sesuai?",
                                  "options": [
                                            "Oligodendroglioma",
                                            "Meningioma",
                                            "Cerebritis",
                                            "Glioma",
                                            "Astrocytoma"
                                  ],
                                  "correctAnswer": 1,
                                  "explanation": "(Meningioma) Massa ekstra-aksial berbatas tegas yang menyangat kuat terhadap kontras dengan gambaran penyebaran dural ( dural tail sign ) adalah karakteristik klasik dari tumor jinak selaput otak (meningioma)."
                        },
                        {
                                  "id": 3,
                                  "question": "Seorang laki-laki berusia 60 tahun datang untuk kontrol rutin setelah mengalami stroke iskemik berat 1 tahun yang lalu. Pasien saat ini mengeluhkan kelemahan sisa pada anggota gerak kanannya. Dilakukan pemeriksaan CT scan kepala tanpa kontras untuk evaluasi. Pada hasil CT scan ditemukan area hipodens fokal dengan densitas menyerupai cairan serebrospinal disertai dilatasi ventrikel ex vacuo di area infark sebelumnya. Apakah gambaran patologi radiologis yang terlihat?",
                                  "options": [
                                            "Encephalomalacia",
                                            "Abscess cerebri",
                                            "Edema sitotoksik difus",
                                            "Perdarahan intraserebral",
                                            "Kalsifikasi distrofik"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "(Encephalomalacia) Ensefalomalasia adalah perlunakan atau hilangnya jaringan otak secara fokal, umumnya merupakan fase kronik/akhir setelah terjadinya infark serebri masa lalu. Ruang yang kosong akan diisi oleh CSF (dilatasi ventrikel ex vacuo )."
                        },
                        {
                                  "id": 4,
                                  "question": "Seorang laki-laki berusia 50 tahun tiba-tiba tidak sadarkan diri dan mengalami henti napas di ruang tunggu rumah sakit. Perawat yang melihat kejadian tersebut langsung mengaktifkan sistem tanggap darurat dan mempersiapkan resusitasi. Berdasarkan protokol kegawatdaruratan medis, berapakah target response time (waktu tanggap) untuk pasien yang memerlukan resusitasi henti jantung?",
                                  "options": [
                                            "segera",
                                            "5 menit",
                                            "15 menit",
                                            "30 menit",
                                            "1 jam"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "(0 detik / segera) Kasus henti jantung adalah kondisi kegawatdaruratan absolut (kategori resusitasi/Triage Merah). Waktu tanggap ( response time ) yang disyaratkan untuk inisiasi tindakan Bantuan Hidup Dasar (BHD/RJP) adalah 0 detik atau harus dikerjakan dengan segera."
                        },
                        {
                                  "id": 5,
                                  "question": "Seorang laki-laki berusia 25 tahun dibawa ke IGD karena tersengat lebah 15 menit yang lalu. Pasien tampak sangat sesak, gelisah, dan memegangi lehernya. Pemeriksaan fisik menunjukkan edema wajah masif, stridor inspiratoar, tekanan darah 80/50 mmHg, nadi 130 x/menit, dan SpO2 85 % pada udara ruang. Apakah tata laksana farmakologis awal yang paling prioritas diberikan?",
                                  "options": [
                                            "Intubasi endotrakeal segera",
                                            "Injeksi ephedrine intravena",
                                            "Observasi klinis berkala",
                                            "Injeksi adrenaline 0,3 mg 1:1000 intramuskular",
                                            "Tracheostomy cito"
                                  ],
                                  "correctAnswer": 3,
                                  "explanation": "(Injeksi adrenaline 0,3 mg 1:1000 intramuskular) Pasien mengalami syok anafilaktik akibat sengatan lebah yang ditandai dengan gangguan jalan napas (stridor, edema wajah) dan hipotensi. Tata laksana lini pertama ( first line ) absolut adalah injeksi Epinefrin/Adrenalin intramuskular."
                        },
                        {
                                  "id": 6,
                                  "question": "Seorang laki-laki berusia 30 tahun datang ke IGD dengan keluhan demam dan nyeri kepala hebat sejak 3 hari yang lalu, disertai perubahan perilaku. Pungsi lumbal menunjukkan pleositosis limfositik dan hasil PCR cairan serebrospinal terkonfirmasi positif untuk Herpes Simplex Virus tipe 1 (HSV-1). Apakah terapi definitif yang paling tepat diberikan pada pasien ini?",
                                  "options": [
                                            "Gentamicin",
                                            "Ampicillin",
                                            "Cefotaxim",
                                            "Acyclovir",
                                            "Dexamethasone"
                                  ],
                                  "correctAnswer": 3,
                                  "explanation": "(Acyclovir) Herpes Simplex Virus tipe 1 (HSV-1) adalah patogen penyebab tersering ensefalitis viral sporadik. Terapi antiviral definitifnya adalah Acyclovir intravena."
                        },
                        {
                                  "id": 7,
                                  "question": "Seorang pasien laki-laki berusia 40 tahun dengan cedera kepala berat (GCS 5) tiba di IGD. Pasien mengalami muntah aktif berulang sehingga dokter memutuskan untuk melakukan intubasi guna mengamankan jalan napas. Manakah prinsip tata laksana airway management yang paling tepat untuk pasien ini?",
                                  "options": [
                                            "Ketamine dipilih jika pasien mengalami hemodinamik yang tidak stabil",
                                            "Midazolam adalah agen pilihan utama karena onsetnya sangat cepat",
                                            "Prosedur intubasi selalu ditunda hingga lambung dikosongkan",
                                            "Pada crash airway wajib diberikan obat pelumpuh otot",
                                            "Intubasi rapid-sequence dilarang pada pasien dengan cedera kepala"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "(Ketamine dipilih jika pasien mengalami hemodinamik yang tidak stabil) Ketamine adalah agen induksi yang memiliki efek simpatomimetik pelepasan katekolamin, sehingga sangat bermanfaat digunakan pada Rapid Sequence Intubation (RSI) untuk pasien syok atau hemodinamik yang tidak stabil karena tidak menurunkan tekanan darah."
                        },
                        {
                                  "id": 8,
                                  "question": "Seorang anak laki-laki berusia 3 tahun dibawa ke IGD dalam kondisi henti napas dan henti jantung. Tim medis segera melakukan resusitasi jantung paru (RJP). Berdasarkan prinsip Airway, Breathing, Circulation (ABC) pada pasien pediatri, apakah etiologi utama yang paling sering mendasari terjadinya cardiac arrest ?",
                                  "options": [
                                            "Kegagalan napas atau hipoksia",
                                            "Aritmia ventrikel primer",
                                            "Kelainan jantung kongenital obstruktif",
                                            "Perdarahan intrakranial masif",
                                            "Sindrom koroner akut"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "(Kegagalan napas atau hipoksia) Berbeda dengan orang dewasa yang etiologi primernya adalah masalah kardiak (iskemia/aritmia), cardiac arrest pada anak/pediatri sebagian besar didahului dan disebabkan oleh gangguan respirasi progesif (hipoksia / gagal napas akut)."
                        },
                        {
                                  "id": 9,
                                  "question": "Seorang anak laki-laki berusia 2 tahun dibawa ke IGD karena kejang kelojotan seluruh tubuh selama 3 menit. Saat tiba di IGD, anak sudah sadar. Suhu tubuh tercatat 39 ° C. Ibu pasien mengatakan bahwa kakak pasien juga memiliki riwayat kejang saat demam. Apakah tatalaksana profilaksis yang paling tepat untuk pasien ini?",
                                  "options": [
                                            "Profilaksis diazepam intermiten saat anak mengalami demam",
                                            "Pemberian diazepam kontinu hingga 2 tahun bebas kejang",
                                            "Pemberian asam valproat kontinu selama 1 tahun bebas kejang",
                                            "Pemberian fenobarbital kontinu selama 1 tahun bebas kejang",
                                            "Pemberian fenitoin kontinu selama 6 bulan bebas kejang"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "(Profilaksis diazepam intermiten saat anak mengalami demam) Pada kasus kejang demam yang perlu pengawasan profilaksis, lini terapi yang direkomendasikan adalah pemberian antikonvulsan intermiten (biasanya Diazepam oral atau per-rektal) hanya pada saat anak sedang mengalami periode demam, disertai pemberian antipiretik."
                        },
                        {
                                  "id": 10,
                                  "question": "Seorang laki-laki berusia 45 tahun datang ke IGD dengan keluhan nyeri kepala hebat, demam tinggi, dan penurunan kesadaran. Pada pemeriksaan fisik didapatkan kaku kuduk ( + ). Hasil analisis cairan serebrospinal menunjukkan peningkatan jumlah leukosit (dominan polimorfonuklear) dan penurunan kadar glukosa. Apakah pemeriksaan penunjang lanjutan yang paling tepat untuk konfirmasi etiologi?",
                                  "options": [
                                            "Kultur gram bakteri dan resistensi antibiotik",
                                            "PCR virus",
                                            "Pemeriksaan KOH",
                                            "Pemeriksaan kadar procalcitonin",
                                            "Pewarnaan tinta India"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "(Kultur gram bakteri dan resistensi antibiotik) Temuan CSS berupa sel leukosit predominan PMN (polimorfonuklear) dan hipoglikorakia (glukosa rendah) mengindikasikan Meningitis Bakterialis. Konfirmasi etiologi patogen definitif ditegakkan melalui pewarnaan gram dan kultur bakteri."
                        },
                        {
                                  "id": 11,
                                  "question": "Seorang laki-laki berusia 55 tahun dibawa ke IGD setelah mengalami kecelakaan lalu lintas ringan. Pasien sadar penuh, tidak ada nyeri dada, sesak, maupun defisit neurologis. Pasien diketahui memiliki riwayat hipertensi yang tidak rutin diobati. Pemeriksaan tanda vital menunjukkan tekanan darah 180/120 mmHg dan pemeriksaan fisik lainnya dalam batas normal. Apakah diagnosis pasien saat ini?",
                                  "options": [
                                            "Hipertensi emergensi",
                                            "Hipertensi krisis",
                                            "Hipertensi urgensi",
                                            "Hipertensi akut",
                                            "Hipertensi kronik"
                                  ],
                                  "correctAnswer": 2,
                                  "explanation": "(Hipertensi urgensi) Hipertensi urgensi ditandai dengan lonjakan tekanan darah sistolik ≥ 180 mmHg atau diastolik ≥ 120 mmHg tanpa disertai gejala atau tanda kerusakan organ target akut (Target Organ Damage)."
                        },
                        {
                                  "id": 12,
                                  "question": "Seorang anak laki-laki berusia 5 tahun dibawa ke IGD dengan keluhan kejang seluruh tubuh. Satu minggu sebelumnya, anak mengeluh buang air kecil berwarna kemerahan (hematuria) dan wajah tampak sembab akibat disfungsi glomerulus paska infeksi. Tekanan darah saat ini 150/100 mmHg. Apakah diagnosis komplikasi yang paling sesuai dengan kondisi neurologis pasien?",
                                  "options": [
                                            "Ensefalopati nefritik",
                                            "Ensefalopati uremikum",
                                            "Hiponatremia akut",
                                            "Ensefalopati iskemik hipoksik",
                                            "Ensefalitis autoimun"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "(Ensefalopati nefritik / Hipertensif) Gejala edema wajah dan hematuria mencerminkan sindrom nefritik akut (seperti GNAPS). Retensi cairan menyebabkan hipertensi yang melonjak, yang pada akhirnya memicu komplikasi hypertensive encephalopathy (ditandai dengan nyeri kepala dan kejang)."
                        },
                        {
                                  "id": 13,
                                  "question": "Seorang laki-laki berusia 35 tahun mengalami dehidrasi berat akibat diare akut. Tubuh mengkompensasi kondisi ini melalui pengaktifan sistem renin-angiotensin-aldosteron (RAAS). Struktur mikroskopis apakah pada ginjal yang memiliki peran utama mendeteksi volume dan aliran darah untuk sekresi renin?",
                                  "options": [
                                            "Sel juxtaglomerular pada arteriol aferen",
                                            "Kelenjar hipofisis posterior",
                                            "Nukleus supraoptik hipotalamus",
                                            "Tubulus kontortus proksimal",
                                            "Ansa Henle pars desendens"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "(Sel juxtaglomerular pada arteriol aferen) Apparatus juxtaglomerular yang berada di struktur arteriol aferen ginjal bertugas sebagai baroreseptor untuk mendeteksi turunnya volume/tekanan darah, yang memicu sintesis dan sekresi renin."
                        },
                        {
                                  "id": 14,
                                  "question": "Seorang laki-laki berusia 65 tahun dibawa ke IGD karena mengalami kejang onset baru yang disertai dengan penurunan kesadaran. Setelah dilakukan resusitasi awal, kesadaran pasien perlahan kembali membaik secara progresif tanpa defisit motorik fokal. Mengingat usia pasien, apakah etiologi primer yang paling sering memicu kondisi tersebut?",
                                  "options": [
                                            "Iskemia serebral",
                                            "Perdarahan subaraknoid",
                                            "Meningitis bakterial",
                                            "Tumor hipofisis",
                                            "Hipoglikemia reaktif"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "(Iskemia serebral) Pada populasi geriatri (di atas 60 tahun), penyebab utama kejang fokal maupun umum new-onset yang paling lazim adalah penyakit serebrovaskular, baik stroke iskemik lama maupun akut."
                        },
                        {
                                  "id": 15,
                                  "question": "Seorang bayi berusia 7 hari dibawa ke fasilitas kesehatan karena kaku seluruh tubuh. Ibu pasien mengatakan bayi sangat sensitif terhadap cahaya dan suara bising yang memicu kejang (spasme). Pada pemeriksaan fisik, terlihat umbilikus kotor dan berbau. Apakah pilihan terapi lini pertama yang paling tepat untuk mengontrol spasme pada pasien ini?",
                                  "options": [
                                            "Diazepam",
                                            "Asam valproat",
                                            "Fenobarbital",
                                            "Karbamazepin",
                                            "Fenitoin"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "(Diazepam) Pada tetanus neonatorum, kejang lebih tepat dideskripsikan sebagai spasme otot rekuren akibat toksin. Terapi lini pertama untuk memutus spasme tersebut adalah benzodiazepin, utamanya Diazepam."
                        },
                        {
                                  "id": 16,
                                  "question": "Seorang perempuan berusia 60 tahun dibawa ke IGD dengan hemiparesis kiri mendadak dan penurunan kesadaran. Pemeriksaan CT scan kepala tanpa kontras menunjukkan lesi hiperdens homogen pada hemisfer kanan yang dikelilingi area hipodens (edema). Terdapat pergeseran garis tengah (midline shift) dan lesi hiperdens di dalam sistem ventrikel. Apakah diagnosis radiologis yang paling tepat?",
                                  "options": [
                                            "Perdarahan intraserebral dekstra akut dan perdarahan intraventrikular akut",
                                            "Perdarahan intraserebral dekstra akut dan perdarahan intraventrikular kronik",
                                            "Infark hemisfer dekstra akut dan perdarahan intraventrikular akut",
                                            "Perdarahan hemisfer dekstra kronik dan hidrosefalus komunikans",
                                            "Tumor intraserebral hemisfer kiri dengan perdarahan masif"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "(Perdarahan intraserebral dekstra akut dan perdarahan intraventrikular akut) Temuan massa hiperdens homogen (darah akut) di jaringan parenkim otak disertai pergeseran garis tengah (efek massa) dan ekstensi densitas serupa ke ruang ventrikel merupakan gambaran khas Intracerebral Hemorrhage (ICH) akut yang ruptur ke sistem ventrikel (IVH)."
                        },
                        {
                                  "id": 17,
                                  "question": "Seorang laki-laki berusia 40 tahun dengan riwayat otitis media supuratif kronis (OMSK) mengeluh nyeri kepala hebat, demam, dan kelemahan sisi tubuh kanan. Dilakukan pemeriksaan CT scan kepala dengan pemberian kontras intravena. Apakah gambaran radiologis khas yang paling mungkin ditemukan?",
                                  "options": [
                                            "Lesi hipodens sentral yang dikelilingi kapsul hiperdens (ring enhancement)",
                                            "Massa hiperdens berbentuk bikonveks di regio temporal",
                                            "Area hipodens berbentuk baji (wedge) mengikuti batas teritori arteri",
                                            "Hiperdensitas yang mengisi ruang sulkus dan sisterna basalis",
                                            "Lesi isointens difus tanpa penyengatan kontras"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "(Lesi hipodens sentral yang dikelilingi kapsul hiperdens/ring enhancement) Abses serebri secara radiologis klasik terlihat sebagai kavitas nekrotik (hipodens di tengah) yang dikelilingi oleh kapsul fibrotik vaskular yang menyerap kontras secara penuh (menyala membentuk cincin / ring enhancement )."
                        },
                        {
                                  "id": 18,
                                  "question": "Seorang pasien dilarikan ke IGD dengan kejang opistotonus hebat setelah tidak sengaja terpapar racun tikus yang dicurigai mengandung zat striknin. Secara farmakologi, bagaimanakah mekanisme kerja toksin tersebut dalam memicu bangkitan kejang?",
                                  "options": [
                                            "Menghambat reseptor GABA",
                                            "Menghambat reseptor glutamat",
                                            "Menghambat reseptor nikotinik",
                                            "Menghambat reseptor kolinergik",
                                            "Menghambat reseptor glisin"
                                  ],
                                  "correctAnswer": 4,
                                  "explanation": "(Menghambat reseptor glisin) Striknin adalah neurotoksin mematikan yang bekerja sebagai antagonis kompetitif pada reseptor Glisin (neurotransmitter inhibitori) di medula spinalis dan batang otak, sehingga menyebabkan aktivitas saraf motorik tak terkendali dan spasme opistotonus."
                        },
                        {
                                  "id": 19,
                                  "question": "Seorang laki-laki berusia 65 tahun datang ke poliklinik dengan keluhan tiba-tiba mengalami mati rasa dan kelumpuhan sementara pada tangan kirinya. Saat serangan terjadi, pasien tetap sadar penuh. Setelah serangan selesai dalam beberapa menit, lengan kirinya terasa sangat lemas (Todd's paresis). Apakah etiologi sekunder yang paling sering mendasari bangkitan fokal pada kelompok usia pasien ini?",
                                  "options": [
                                            "Epilepsi idiopatik",
                                            "Hiponatremia",
                                            "Hipoksia serebri",
                                            "Stroke iskemik/hemoragik",
                                            "Kejang demam"
                                  ],
                                  "correctAnswer": 3,
                                  "explanation": "(Stroke iskemik/hemoragik) Bangkitan fokal yang diikuti fenomena Todd's paresis (kelemahan lokal sementara setelah kejang) pada usia lanjut paling sering dilatarbelakangi oleh lesi struktural kortikal, dan penyebab tersering kelainan tersebut pada usia lansia adalah stroke (penyakit serebrovaskular)."
                        },
                        {
                                  "id": 20,
                                  "question": "Seorang perempuan berusia 30 tahun sedang dievaluasi di unit pemantauan epilepsi. Saat bangkitan kejang fokal dimulai, manifestasi klinis pertama yang selalu muncul adalah kedua matanya dan kepalanya secara involunter memutar jauh ke satu sisi (adversive seizure). Di area korteks serebri manakah letak lokus epileptogenik yang paling sesuai dengan gejala ini?",
                                  "options": [
                                            "Area Brodmann 1, 2, 3",
                                            "Area Brodmann 4",
                                            "Area Brodmann 5, 7",
                                            "Area Brodmann 6",
                                            "Area Brodmann 8"
                                  ],
                                  "correctAnswer": 4,
                                  "explanation": "(Area Brodmann 8) Area Brodmann 8 adalah Frontal Eye Field (FEF). Eksitasi atau cetusan epileptogenik dari lesi di area ini secara klasik akan menyebabkan mata melirik menjauhi lokasi lesi (deviasi konjugat kontralateral / versi adversif) disertai putaran kepala."
                        },
                        {
                                  "id": 21,
                                  "question": "Seorang perempuan berusia 45 tahun datang ke IGD dengan penurunan kesadaran progresif. Pemeriksaan fisik menunjukkan tekanan darah 170/110 mmHg dan edema ringan pada kedua tungkai. Hasil laboratorium mengonfirmasi adanya hipoalbuminemia berat dan penurunan fungsi laju filtrasi glomerulus. Kondisi patologis neurologis apakah yang paling mungkin sedang terjadi?",
                                  "options": [
                                            "Ensefalopati nefrotik",
                                            "Gagal ginjal akut",
                                            "Ensefalopati uremikum",
                                            "Transient Ischemic Attack",
                                            "Meningoensefalitis tuberkulosa"
                                  ],
                                  "correctAnswer": 2,
                                  "explanation": "(Ensefalopati uremikum) (Sesuai opsi terdekat dari fungsi ginjal) Edema dan penurunan laju filtrasi glomerulus dengan kemungkinan penumpukan toksin uremik (akibat penurunan fungsi filtrasi berat) dapat mengiritasi sistem saraf pusat, memicu ensefalopati uremikum. (Jika krisis hipertensinya yang ditonjolkan, Hypertensive encephalopathy juga bisa dipertimbangkan jika tersedia di soal asli)."
                        },
                        {
                                  "id": 22,
                                  "question": "Seorang bayi berusia 8 hari didiagnosis mengalami tetanus neonatorum akibat persalinan dengan alat yang tidak steril. Bayi mengalami spasme otot rahang yang parah (trismus). Obat lini pertama apakah yang merupakan drug of choice untuk mengendalikan spasme otot pada kasus ini?",
                                  "options": [
                                            "Fenobarbital",
                                            "Diazepam",
                                            "Fenitoin",
                                            "Karbamazepin",
                                            "Gabapentin"
                                  ],
                                  "correctAnswer": 1,
                                  "explanation": "(Diazepam) Sama dengan konsep nomor 15; manajemen utama simtomatik untuk mengendalikan rigiditas dan spasme muskular berat yang dipicu rangsangan pada penyakit Tetanus adalah pelemas otot sentral seperti Diazepam intravena."
                        },
                        {
                                  "id": 23,
                                  "question": "Seorang pasien laki-laki berusia 45 tahun yang mengalami gagal napas akan segera diintubasi di ruang gawat darurat. Dokter anestesi berencana menggunakan agen induksi intravena yang menjaga stabilitas hemodinamik secara optimal dan memiliki masa kerja yang singkat. Manakah obat induksi yang paling sesuai dengan deskripsi tersebut?",
                                  "options": [
                                            "Ketamine",
                                            "Etomidate",
                                            "Midazolam",
                                            "Propofol",
                                            "Fentanyl"
                                  ],
                                  "correctAnswer": 1,
                                  "explanation": "(Etomidate) Etomidate adalah agen hipnotik anestesi intravena yang sangat disenangi untuk intubasi dan tata laksana jalan napas darurat karena kerjanya sangat cepat dan hampir tidak menyebabkan depresi kardiovaskular (sangat stabil untuk hemodinamik)."
                        },
                        {
                                  "id": 24,
                                  "question": "Seorang anak berusia 4 tahun mengalami kejang tonik-klonik di rumah. Tenaga medis yang datang ke lokasi mengalami kesulitan dalam melakukan pemasangan akses intravena akibat pergerakan pasien. Dari berbagai rute alternatif pemberian antikonvulsan darurat, rute manakah yang mutlak dikontraindikasikan (dihindari) saat pasien sedang aktif kejang?",
                                  "options": [
                                            "Per-oral",
                                            "Per-rectal",
                                            "Subcutan",
                                            "Buccal",
                                            "Intranasal"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "(Intraoral / per-oral) Memberikan obat secara per-oral (ditelan melalui mulut) pada pasien yang sedang tidak sadar atau sedang kejang kelojotan aktif sangat dilarang karena tingginya risiko tersedak dan aspirasi ke dalam paru-paru."
                        },
                        {
                                  "id": 25,
                                  "question": "Seorang laki-laki berusia 65 tahun dibawa ke IGD dalam kondisi somnolen. Keluarga menyatakan pasien memiliki riwayat diabetes melitus yang tidak terkontrol, serta belakangan ini buang air kecilnya sangat sedikit disertai pembengkakan pada kedua kaki (edema). Untuk melacak penyebab metabolik penurunan kesadaran pada kasus ini, pemeriksaan darah spesifik apakah yang paling tepat diusulkan?",
                                  "options": [
                                            "Hitung jumlah leukosit",
                                            "Hitung jumlah eritrosit",
                                            "Pemeriksaan kadar ureum dan kreatinin",
                                            "Laju endap darah",
                                            "Kadar bilirubin total"
                                  ],
                                  "correctAnswer": 2,
                                  "explanation": "(Pemeriksaan kadar ureum dan kreatinin) Riwayat DM kronis tidak terkontrol dengan keluhan oliguria (kencing sedikit), edema, dan napas bau uremik"
                        }
              ],
              "2023": [
                        {
                                  "id": 1,
                                  "question": "Seorang pasien dibawa ke poliklinik saraf karena sering mengalami kehilangan kesadaran secara mendadak. Dokter mencurigai adanya sindrom epilepsi. Berikut ini yang merupakan ciri khas pasti yang selalu terdapat pada manifestasi kejang (seizure) adalah?",
                                  "options": [
                                            "Gerakan otot tonik-klonik",
                                            "Hilangnya kesadaran singkat yang ditandakan dengan kondisi 'petit mal'",
                                            "Gangguan gerakan bola mata",
                                            "Perubahan tampakan gelombang otak",
                                            "Gerakan kaki menghentak yang ekstrem"
                                  ],
                                  "correctAnswer": 3,
                                  "explanation": "Perubahan tampakan gelombang otak Kejang secara definisi medis adalah manifestasi klinis dari abnormalitas pelepasan muatan listrik sel neuron di otak secara berlebihan dan sinkron, yang secara pasti akan bermanifestasi pada rekaman aktivitas gelombang otak (EEG)."
                        },
                        {
                                  "id": 2,
                                  "question": "Seorang pria berusia 35 tahun dibawa ke IGD oleh keluarganya dalam kondisi kejang seluruh tubuh. Saat tiba di brankar, pasien masih mengalami kejang tonik-klonik dan tidak merespons panggilan. Langkah pertama yang paling krusial untuk dilakukan oleh dokter jaga adalah?",
                                  "options": [
                                            "Berikan diazepam rectal 10 mg",
                                            "Periksa airway, breathing, circulation",
                                            "Hentikan kejang dan mengamankan jalan napas",
                                            "Injeksi phenytoin 15-20 mg/kgBB",
                                            "Periksa gelombang EEG dan melakukan cEEG"
                                  ],
                                  "correctAnswer": 1,
                                  "explanation": "Periksa airway, breathing, circulation Dalam setiap tatalaksana kegawatdaruratan, termasuk kegawatdaruratan neurologis seperti kejang atau status epileptikus, penilaian dan patensi prinsip ABC (Airway, Breathing, Circulation) selalu merupakan langkah perintis (langkah pertama) sebelum intervensi farmakologis."
                        },
                        {
                                  "id": 3,
                                  "question": "Seorang anak perempuan berusia 6 bulan dibawa ke IGD karena riwayat kejang di rumah. Kejang dilaporkan hanya terjadi pada lengan dan tangan kanan. Selama kejang anak tetap sadar dan durasi kejang sekitar 2 menit. Ini merupakan kejang dengan gejala yang sama untuk ketiga kalinya. Suhu tubuh diukur 40 ° C. Satu hari sebelumnya anak mulai mengalami demam tinggi dan batuk pilek. Pemeriksaan tanda meningeal negatif. Apakah terapi pemeliharaan yang paling tepat diperlukan?",
                                  "options": [
                                            "Terapi intermitten dengan pemberian acetaminophen dan diazepam rectal pada saat demam",
                                            "Diberikan profilaksis kejang dengan phenobarbital selama 1 tahun",
                                            "Pemberian antibiotik spektrum luas dan diazepam rectal",
                                            "Pemberian asam valproat per oral sebagai terapi simptomatik",
                                            "Observasi ketat tanda vital tanpa medikasi profilaksis"
                                  ],
                                  "correctAnswer": 1,
                                  "explanation": "Diberikan profilaksis kejang dengan phenobarbital selama 1 tahun Pasien memiliki manifestasi kejang demam kompleks (bersifat fokal pada ekstremitas kanan dan berulang ketiga kalinya/multipel dalam periode singkat). Tatalaksana lanjutan untuk KDK (Kejang Demam Kompleks) adalah terapi rumatan/profilaksis kontinu, di mana asam valproat dan fenobarbital merupakan pilihan profilaksis utama selama 1 tahun bebas kejang."
                        },
                        {
                                  "id": 4,
                                  "question": "Seorang bayi lahir spontan pada usia gestasi 40 minggu dengan riwayat distosia dan asfiksia perinatal. Sesaat setelah lahir, bayi tidak langsung menangis, tonus otot lemas, dan terdapat penurunan kesadaran yang nyata. Berdasarkan temuan klinis tersebut, ensefalopati jenis apakah yang paling mungkin dialami bayi tersebut?",
                                  "options": [
                                            "Septic-associated encephalopathy (SAE)",
                                            "Ensefalopati metabolik",
                                            "Ensefalopati uremikum",
                                            "Ensefalopati pulmonal",
                                            "Hypoxic Ischemic Encephalopathy (HIE)"
                                  ],
                                  "correctAnswer": 4,
                                  "explanation": "Hypoxic Ischemic Encephalopathy (HIE) Bayi dengan asfiksia perinatal (tidak menangis, tonus lemas) sangat rentan mengalami hipoksia otak akibat asfiksia yang berkelanjutan. Hal ini menyebabkan ensefalopati hipoksik iskemik (HIE)."
                        },
                        {
                                  "id": 5,
                                  "question": "Seorang pasien dengan status epileptikus refrakter diberikan injeksi midazolam kontinyu setelah gagal berespons terhadap tatalaksana lini pertama dan kedua. Berdasarkan mekanisme farmakologisnya, midazolam termasuk dalam golongan antikonvulsan jenis?",
                                  "options": [
                                            "Gabapentinoid",
                                            "Benzodiazepine",
                                            "Topiramate",
                                            "Hidantoin",
                                            "Carbamazepine"
                                  ],
                                  "correctAnswer": 1,
                                  "explanation": "Benzodiazepine Midazolam, diazepam, lorazepam, dan clonazepam semuanya merupakan obat-obatan penekan sistem saraf pusat yang masuk ke dalam kelas farmakologis Benzodiazepine."
                        },
                        {
                                  "id": 6,
                                  "question": "Seorang pria berusia 45 tahun korban kecelakaan lalu lintas dengan riwayat trauma kepala berat dirawat di ICU. Pada hari ketiga, pasien mengalami ensefalopati dengan letargi. Hasil pemeriksaan laboratorium menunjukkan kadar natrium serum 130 mEq/L. Kemungkinan penyebab sekunder ensefalopati metabolik yang dialami pasien tersebut adalah?",
                                  "options": [
                                            "Hypoxic encephalopathy",
                                            "Hepatic encephalopathy",
                                            "Uremic encephalopathy",
                                            "Hyponatremic encephalopathy",
                                            "Hypernatremic encephalopathy"
                                  ],
                                  "correctAnswer": 3,
                                  "explanation": "Hyponatremic encephalopathy Nilai kadar natrium darah normal adalah 135-145 mEq/L. Kadar 130 mEq/L menunjukkan kondisi hiponatremia. Pada level yang lebih rendah, kondisi ini dapat memicu edema seluler di parenkim otak, menyebabkan ensefalopati hiponatremik. (Catatan: Modifikasi opsi dibuat agar sinkron dengan data klinis dari nilai 130 mEq/L yang merupakan hyponatremia)."
                        },
                        {
                                  "id": 7,
                                  "question": "Seorang anak laki-laki berusia 1 tahun dibawa ke IGD dengan keluhan demam tinggi, rewel, dan kejang. Pada pemeriksaan fisik ditemukan kaku kuduk dan tanda meningeal positif. Dokter merencanakan pungsi lumbal namun segera memulai terapi empiris curiga meningitis bakterialis. Antibiotik lini pertama yang paling tepat untuk anak tersebut adalah?",
                                  "options": [
                                            "Metronidazole 500 mg IV",
                                            "Ceftriaxone 200 mg/kgBB IV",
                                            "Cefotaxime 300 mg/kgBB IV",
                                            "Ampicilin 2 g IV",
                                            "Gentamicin 5 mg/kgBB IV"
                                  ],
                                  "correctAnswer": 2,
                                  "explanation": "Cefotaxime 300 mg/kgBB IV Lini pertama tata laksana empirik untuk kecurigaan meningitis pada anak (usia > 1 bulan hingga 5 tahun) adalah Sefalosporin generasi ketiga. Dosis rekomendasi empiris Cefotaxime pada meningitis anak adalah dosis tinggi mencapai 200-300 mg/kgBB/hari terbagi, atau Ceftriaxone 100 mg/kgBB. Pilihan B (Ceftriaxone 200 mg/kgBB) bersifat sangat melebihi dosis toksik lazim, sehingga Cefotaxime 300 mg/kgBB/hari lebih tepat mewakili rentang tata laksana."
                        },
                        {
                                  "id": 8,
                                  "question": "Seorang perempuan berusia 20 tahun dibawa ke IGD dengan penurunan kesadaran, hipersalivasi, dan pupil miotik. Keluarganya menyatakan pasien meminum racun serangga setelah putus cinta. Patofisiologi utama yang mengakibatkan penurunan kesadaran pada toksisitas organofosfat tersebut adalah?",
                                  "options": [
                                            "Locked-in syndrome",
                                            "Gangguan metabolisme otak",
                                            "Gangguan neurotransmitter otak",
                                            "Gangguan aliran darah",
                                            "Edema serebri yang mengakibatkan penekanan parenkim"
                                  ],
                                  "correctAnswer": 2,
                                  "explanation": "Gangguan neurotransmitter otak Baygon mengandung agen organofosfat (inhibitor kolinesterase). Intoksikasi bahan ini mencegah pemecahan asetilkolin yang menyebabkan akumulasi neurotransmitter asetilkolin berlebihan di celah sinaps (krisis kolinergik), yang merepresentasikan gangguan mekanisme neurotransmitter."
                        },
                        {
                                  "id": 9,
                                  "question": "Seorang wanita berusia 40 tahun datang ke IGD dengan keluhan kelemahan otot ekstremitas yang progresif setelah mengalami diare profus selama 3 hari. Pada pemeriksaan EKG ditemukan gambaran gelombang U yang prominen (menonjol) setelah gelombang T. Kondisi gangguan elektrolit yang mendasari gambaran EKG tersebut adalah?",
                                  "options": [
                                            "Hypernatremia",
                                            "Hyponatremia",
                                            "Hypoglycemia",
                                            "Hyperkalemia",
                                            "Hypokalemia"
                                  ],
                                  "correctAnswer": 4,
                                  "explanation": "Hypokalemia Temuan paling klasik dari hipokalemia berat pada EKG adalah pemanjangan interval QU, munculnya gelombang U yang prominen (menonjol) pasca gelombang T, serta depresi segmen ST sekunder dan gelombang T yang merata."
                        },
                        {
                                  "id": 10,
                                  "question": "Seorang bayi baru lahir cukup bulan via persalinan spontan pervaginam. Pada pemeriksaan fisik awal, bayi tampak hipotonia berat, makroglosia, dan memiliki fasies khas seperti gambaran sindrom Down. Tidak ada riwayat asfiksia perinatal. Apa kemungkinan etiologi penyebab kelainan kongenital pada bayi tersebut?",
                                  "options": [
                                            "Defisiensi hormon tiroid kongenital",
                                            "Asfiksia perinatal",
                                            "Infeksi rubella kongenital",
                                            "Defisiensi vitamin B-1",
                                            "Kelainan anatomi fossa posterior"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "Defisiensi hormon tiroid kongenital Hipotiroidisme kongenital memiliki gambaran klinis khas yang mencakup makroglosia (lidah membesar), hipotonia otot, kulit yang burik/kering, hernia umbilikalis, dan wajah sembap (myxedema) yang sering keliru dianggap sebagai gambaran sindrom Down kongenital."
                        },
                        {
                                  "id": 11,
                                  "question": "Seorang pria 55 tahun tiba-tiba hemiparesis mendadak dan segera dibawa ke IGD. CT scan awal mengonfirmasi diagnosis stroke hemorrhagic hiperakut ( < 24 jam). Jika pada pasien ini dilakukan pemeriksaan MRI kepala, intensitas sinyal (menggambarkan oksihemoglobin) yang akan tampak adalah?",
                                  "options": [
                                            "T1: isointense, T2: iso to hyperintense, DWI: high, ADC: low",
                                            "T1: isointense to hypointense, T2: hypointense, DWI: low, ADC: low",
                                            "T1: isointense, T2: hyperintense, DWI: low, ADC: high",
                                            "T1: isointense, T2: hyperintense, FLAIR: variable, DWI: high",
                                            "T1: hypointense, T2: hyperintense, FLAIR: hyperintense, ADC: low"
                                  ],
                                  "correctAnswer": 0,
                                  "explanation": "T1: isointense, T2: iso to hyperintense, DWI: high, ADC: low Pada fase hiperakut dari perdarahan (Hemorrhagic stroke < 24 jam), komponen sel darah merah intraparenkimal sebagian besar terisi oleh Oxyhemoglobin . Pada MRI, fase ini umumnya memberikan gambaran sinyal isointense pada sekuens T1-weighted, serta bervariasi iso- hiperintense pada T2-weighted."
                        },
                        {
                                  "id": 12,
                                  "question": "Seorang pasien wanita 65 tahun dicurigai mengalami stroke iskemik akut kurang dari 6 jam pasca onset. Pemeriksaan CT Scan awal tampak normal tanpa tanda perdarahan. Untuk mengonfirmasi infark iskemik akut, dilakukan MRI. Gambaran khas infark iskemik akut yang paling sesuai pada sekuens MRI adalah?",
                                  "options": [
                                            "T1: isointense, T2: hypointense, FLAIR: hypointense, DWI: high",
                                            "T1: isointense, T2: hyperintense, FLAIR: hyperintense, DWI: high",
                                            "T1: hypointense, T2: hyperintense, FLAIR: hyperintense, DWI: high",
                                            "T1: hypointense, T2: hypointense, FLAIR: hyperintense, DWI: low",
                                            "Tidak tampak adanya contrast enhancement"
                                  ],
                                  "correctAnswer": 2,
                                  "explanation": "T1: hypointense, T2: hyperintense, FLAIR: hyperintense, DWI: high Karakteristik paling sensitif dari infark iskemik akut pada modalitas MRI adalah restricted diffusion yang memunculkan sinyal sangat tinggi (hyperintense/high) pada fase Diffusion Weighted Imaging (DWI), disertai gambaran lesi isointense-hipointense di T1, dan munculnya edema hiperintens pada sekuens pembobotan air (T2 dan FLAIR)."
                        },
                        {
                                  "id": 13,
                                  "question": "Seorang pria berusia 60 tahun dengan riwayat diabetes mellitus kronis dibawa ke rumah sakit karena letargi dan mioklonus. Pemeriksaan napas tercium bau urin (uremic fetor). Pasien didiagnosis mengalami ensefalopati uremikum. Kondisi manakah di bawah ini yang paling menandakan kriteria awitan umum penyakit tersebut?",
                                  "options": [
                                            "Peningkatan eGFR ( > 90 mL/menit)",
                                            "eGFR menurun dan menetap di bawah 15 mL/menit",
                                            "Kadar natrium darah > 145 mEq/L",
                                            "Riwayat gagal ginjal kronis tidak mempengaruhi",
                                            "Penurunan kadar serum kreatinin"
                                  ],
                                  "correctAnswer": 1,
                                  "explanation": "eGFR menurun dan menetap di bawah 15 mL/menit Ensefalopati uremikum umumnya bermanifestasi jika fungsi ginjal pasien telah masuk ke derajat akhir atau End Stage Renal Disease (ESRD) yang secara fungsi ditandai dengan klirens kreatinin atau laju filtrasi glomerulus yang jatuh hingga di bawah 15 mL/menit."
                        },
                        {
                                  "id": 14,
                                  "question": "Seorang wanita berusia 50 tahun didiagnosis menderita tumor intrakranial. Hasil pencitraan MRI menunjukkan massa tumor yang secara spesifik berlokasi di daerah ganglia basal. Dalam menentukan rencana tatalaksana operatif bersama keluarga, apa yang menjadi pertimbangan atau kendala penyulit utama untuk reseksi tumor tersebut?",
                                  "options": [
                                            "Ukuran",
                                            "Derajat",
                                            "Usia",
                                            "Lokasi",
                                            "Respons terhadap kemoterapi"
                                  ],
                                  "correctAnswer": 3,
                                  "explanation": "Lokasi Ganglia basal terletak pada zona deep brain yang dikelilingi traktus jaras kortikospinal krusial dan arteri lentikulostriata. Akses bedah terbuka menuju area ini menanggung beban risiko defisit neurologis ireversibel yang luar biasa masif (morbiditas dan mortalitas pasca operasi yang tinggi). Lokasi inilah yang menjadi penentu atau kontraindikasi relatif dilakukannya reseksi total."
                        },
                        {
                                  "id": 15,
                                  "question": "Seorang pemuda 22 tahun dicurigai kuat mengalami meningitis bakterialis akut. Dokter jaga segera melakukan tindakan pungsi lumbal. Saat aspirasi, cairan serebrospinal (CSF) yang keluar tampak keruh secara makroskopis. Temuan pada analisis CSF manakah di bawah ini yang merupakan penyebab paling spesifik dari kekeruhan cairan tersebut?",
                                  "options": [
                                            "RBC yang lisis",
                                            "Hyperbilirubinemia ( > 6 mg/dL)",
                                            "Leukositosis dalam CSF ( > 200 sel/ μ L)",
                                            "RBC < 400 sel/ μ L",
                                            "Penurunan kadar protein"
                                  ],
                                  "correctAnswer": 2,
                                  "explanation": "Leukositosis dalam CSF ( > 200 sel/ μ L) Pungsi lumbal yang menghasilkan cairan keruh (turbid / purulen) adalah hallmark dari meningitis bakterialis. Secara makroskopik, kekeruhan utamanya secara signifikan berkolerasi dengan pleositosis / leukositosis yang parah, yaitu penumpukan dominan sel polimorfonuklear di atas > 200 sel/ μ L"
                        }
              ]
            }
          },
          "iup": {
            name: "IUP",
            years: {
              "2021": [
              {
                "id": 1,
                "question": "Seorang pasien datang dengan keluhan kejang fokal yang diawali dengan gerakan kepala dan mata menoleh secara involunter ke satu sisi. Hasil pencitraan menunjukkan adanya lesi pada lobus frontalis kiri. Area fungsional manakah yang paling mungkin memicu pola kejang tersebut?",
                "options": ["Area Brodmann 4", "Area Brodmann 5, 7", "Area Brodmann 3, 1, 2", "Area Brodmann 6", "Area Brodmann 8"],
                "correctAnswer": 4,
                "explanation": "Area Brodmann 8 (Frontal Eye Field) berada di lobus frontalis. Stimulasi atau lesi iritatif (seperti kejang) di area ini menyebabkan mata dan kepala deviasi ke arah kontralateral."
              },
              {
                "id": 2,
                "question": "Seorang pria berusia 45 tahun datang ke klinik dengan keluhan pusing berputar yang memberat saat perubahan posisi kepala. Dokter meresepkan medikamentosa yang diketahui memiliki mekanisme kerja menghambat reseptor histamin sentral sekaligus menekan aktivitas kolinergik. Obat manakah yang dimaksud?",
                "options": ["Dimenhidrinat", "Diazepam", "Fenitoin", "Hiosin", "Betahistin"],
                "correctAnswer": 0,
                "explanation": "Dimenhidrinat adalah obat golongan antihistamin H1 yang juga memiliki sifat antikolinergik sentral yang kuat, sering digunakan untuk menekan respon vestibular."
              },
              {
                "id": 3,
                "question": "Seorang neonatus dibawa ke IGD karena status epileptikus. Pasien telah menerima loading dose fenobarbital 20 mg/kgBB intravena, namun kejang secara klinis masih terus berlanjut. Langkah farmakologis manakah yang paling tepat untuk dipertimbangkan selanjutnya menurut algoritma?",
                "options": ["Fenobarbital 35 mg", "Fenobarbital 70 mg", "Diazepam 35 mg", "Diazepam 70 mg", "Fenitoin 70 mg"],
                "correctAnswer": 1,
                "explanation": "Berdasarkan opsi pada soal, dosis fenobarbital dapat diulang dengan dosis bervariasi bergantung berat badan jika kejang belum teratasi (opsi persis dari sumber aslinya)."
              },
              {
                "id": 4,
                "question": "Seorang pasien dengan cedera kepala berat direncanakan untuk dilakukan intubasi endotrakeal secara Rapid Sequence Intubation (RSI). Dokter anestesi memilih propofol sebagai agen induksi. Manakah dari pernyataan berikut yang merupakan karakteristik klinis utama dari obat tersebut?",
                "options": ["Menyebabkan nyeri pada lokasi injeksi", "Meningkatkan tekanan intrakranial", "Memicu bronkospasme berat", "Memiliki efek analgesik yang adekuat", "Meningkatkan laju metabolisme otak"],
                "correctAnswer": 0,
                "explanation": "Propofol terkenal menyebabkan nyeri tajam saat disuntikkan ke vena perifer akibat formulasi emulsi lipidnya. Obat ini justru menurunkan tekanan intrakranial dan laju metabolisme otak."
              },
              {
                "id": 5,
                "question": "Dalam penatalaksanaan pasien henti jantung di ruang gawat darurat, tim medis melakukan resusitasi jantung paru (RJP). Terkait timeline atau prinsip tindakan resusitasi primer pada pasien henti jantung, manakah pernyataan yang paling tepat?",
                "options": ["Tindakan dilakukan secara kontinu dan simultan pada menit ke-0", "Evaluasi status sirkulasi definitif ditunda hingga 1 jam", "Pemberian kompresi dada ditunda hingga akses jalan napas terpasang", "Intervensi dihentikan otomatis setelah 10 menit tanpa melihat ritme", "Intubasi endotrakeal menjadi prioritas pertama mendahului kompresi"],
                "correctAnswer": 0,
                "explanation": "Resusitasi kardiopulmoner modern (RJP/CPR) menekankan inisiasi kompresi dengan jeda seminimal mungkin (kontinu) dan dilakukan segera (menit ke-0)."
              },
              {
                "id": 6,
                "question": "Seorang pasien laki-laki berusia 55 tahun menjalani pemeriksaan EKG rutin. Pada pembacaan EKG, ditemukan adanya gambaran gelombang P yang sesekali hilang (dropped P wave), di mana interval P-P yang panjang merupakan kelipatan matematis pasti dari siklus P-P dasar. Apakah diagnosis aritmia yang paling tepat?",
                "options": ["Sinoatrial block first degree", "Sinoatrial block second degree", "Sinoatrial block third degree", "Sinus bradikardia", "Atrioventricular block second degree type 1"],
                "correctAnswer": 1,
                "explanation": "Pada SA block derajat 2, impuls tidak keluar dari nodus SA sehingga gelombang P dan kompleks QRS hilang, membentuk jeda yang merupakan kelipatan eksak dari interval P-P sebelumnya."
              },
              {
                "id": 7,
                "question": "Seorang perempuan berusia 60 tahun dengan riwayat End Stage Renal Disease (ESRD) dibawa ke IGD karena letargi dan kebingungan. Pemeriksaan fisik menunjukkan TD 160/90 mmHg, HR 110 x/menit, dan RR 10 x/menit. Hasil laboratorium menunjukkan kadar BUN 101 mg/dL dan kreatinin 9,2 mg/dL. Apakah diagnosis yang paling mungkin?",
                "options": ["Ensefalopati uremikum", "Ensefalopati hipertensif", "Ensefalopati hepatikum", "Ensefalopati Wernicke", "Stroke iskemik akut"],
                "correctAnswer": 0,
                "explanation": "Kadar BUN yang sangat tinggi (101 mg/dL) dan kreatinin (9.2) pada pasien gagal ginjal dengan penurunan kesadaran sangat khas untuk intoksikasi uremik (ensefalopati uremikum)."
              },
              {
                "id": 8,
                "question": "Seorang bayi berusia 2 bulan dievaluasi di NICU karena dismorfia wajah, letargi persisten, dan kejang. Tim medis mencurigai adanya inborn error of metabolism yang melibatkan defek enzim spesifik. Apakah patomekanisme dasar yang paling memicu terjadinya ensefalopati pada kondisi tersebut?",
                "options": ["Cedera sitotoksik akibat penumpukan metabolit", "Interupsi fisik pada jaras polisinaptik", "Inflamasi neuronal masif", "Gangguan arsitektur pembuluh darah serebral", "Penumpukan plak amiloid pada ruang ekstraseluler"],
                "correctAnswer": 0,
                "explanation": "Inborn error of metabolism menyebabkan akumulasi zat toksik intraseluler atau defisit energi yang pada akhirnya merusak pompa ion sel saraf, memicu edema sitotoksik ensefalopati."
              },
              {
                "id": 9,
                "question": "Saat memimpin pelatihan Pediatric Advanced Life Support (PALS), seorang instruktur menekankan etiologi spesifik yang membedakan penanganan pasien dewasa dan anak. Manakah dari pernyataan berikut yang menjadi dasar epidemiologis pertimbangan Airway- Breathing-Circulation pada anak?",
                "options": ["Angka obesitas pada anak semakin meningkatkan risiko penyumbatan napas", "Sekitar 80% kasus henti jantung pada anak diawali oleh masalah pernapasan", "Sebagian besar henti jantung pada anak disebabkan oleh masalah kardiovaskular primer", "Sebagian besar henti jantung pada dewasa murni disebabkan oleh obstruksi pernapasan", "Anatomi jalan napas anak tidak memiliki modifikasi signifikan dibandingkan dewasa"],
                "correctAnswer": 1,
                "explanation": "Henti jantung pediatrik umumnya bersifat sekunder akibat hipoksia (asfiksia/masalah respirasi), bukan henti jantung mendadak karena iskemik kardiovaskular seperti pada orang dewasa."
              },
              {
                "id": 10,
                "question": "Seorang pasien dewasa dengan demam dan kaku kuduk dicurigai mengalami meningitis. Dokter melakukan prosedur pungsi lumbal dan mendapati cairan serebrospinal (CSF) berwarna keruh (turbid) secara makroskopis. Kondisi mikroskopis atau etiologis manakah yang paling sesuai dengan gambaran tersebut?",
                "options": ["Peningkatan leukosit 50 sel/mm3", "Peningkatan eritrosit 100 sel/mm3", "Kontaminasi jarum suntik", "Infeksi virus", "Infeksi bakteri"],
                "correctAnswer": 4,
                "explanation": "Kekeruhan (turbiditas) makroskopis pada CSF umumnya diakibatkan oleh purulensi, yang menandakan tingginya kadar leukosit PMN (biasanya ribuan) akibat infeksi bakteri."
              },
              {
                "id": 11,
                "question": "Seorang pasien epilepsi disarankan oleh dokter saraf untuk melakukan pemantauan kadar obat darah secara berkala dan berhati-hati dalam merubah dosis mandiri, mengingat obat yang dikonsumsi memiliki profil farmakokinetik zero-order dan narrow therapeutic window. Obat manakah yang dimaksud?",
                "options": ["Fenitoin", "Levetirasetam", "Tiopental", "Fenobarbital", "Okskarbazepin"],
                "correctAnswer": 0,
                "explanation": "Fenitoin memiliki profil kinetika orde-nol (zero-order/nonlinear) dan rentang terapeutik sempit, sehingga perubahan dosis sedikit saja dapat memicu intoksikasi."
              },
              {
                "id": 12,
                "question": "Seorang pasien pria berusia 35 tahun dicurigai mengalami infeksi intrakranial difus. Dokter merencanakan pemeriksaan Magnetic Resonance Imaging (MRI) kepala dengan kontras. Apabila patologi pasien berada tepat pada fase early cerebritis, gambaran radiologis apakah yang paling mungkin teridentifikasi?",
                "options": ["Lesi hipointens berbatas tegas pada sekuens T2", "Lesi hiperintens merata pada sekuens T1", "Lesi hipointens pada T1, hiperintens pada T2, disertai edema", "Lesi hipointens dan hiperintens pada T1 dengan perifokal edema yang kalsifikasi", "Gambaran ring-enhancement tebal yang berbatas tegas pasca-kontras"],
                "correctAnswer": 2,
                "explanation": "Pada fase early cerebritis jaringan otak meradang difus tanpa kapsul, tampak sebagai area batas tak tegas, hipointens di T1, hiperintens di T2 (menandakan air/edema). Ring enhancement baru muncul di fase abses kapsuler."
              },
              {
                "id": 13,
                "question": "Seorang bayi berusia 14 bulan dibawa ke IGD setelah mengalami kejang kelojotan seluruh tubuh. Orang tua melaporkan bayi demam tinggi sejak semalam disertai batuk, dengan suhu tiba di IGD $40^{\circ}$C. Saat diperiksa pasien telah sadar (compos mentis) dan tidak ada tanda rangsang meningeal. Riwayat menunjukkan kejang ini merupakan episode kedua dalam 24 jam terakhir dengan pola yang sama. Apakah diagnosis yang paling tepat?",
                "options": ["Kejang demam kompleks", "Kejang demam sederhana", "Epilepsi fokal berulang", "Febrile provoked epilepsy", "Meningoensefalitis viral"],
                "correctAnswer": 0,
                "explanation": "Karena kejang terjadi lebih dari 1 kali (dua kali) dalam durasi 24 jam dengan adanya provokasi demam tanpa infeksi intrakranial, diagnosisnya adalah kejang demam kompleks."
              },
              {
                "id": 14,
                "question": "Seorang anak dengan sindrom Lennox-Gastaut sering mengalami kejang generalisata tipe drop attacks yang menyebabkan cedera kepala berulang. Medikamentosa tidak memberikan perbaikan. Dokter bedah merencanakan operasi paliatif untuk mencegah sinkronisasi atau penyebaran aktivitas listrik kejar antarkedua hemisfer serebri. Prosedur bedah apakah yang paling direkomendasikan?",
                "options": ["Kalosotomi", "Hemisferektomi", "Lobektomi anterior", "Lesionektomi fokal", "Stimulasi nervus vagus"],
                "correctAnswer": 0,
                "explanation": "Corpus callosotomy memotong korpus kalosum (penghubung hemisfer kiri dan kanan), yang bertujuan memutus jalur penyebaran kejang generalisata yang menyebabkan drop attacks."
              },
              {
                "id": 15,
                "question": "Seorang bayi berusia 8 bulan dibawa ke rumah sakit dengan tanda klinis syok hipovolemik akibat diare akut dehidrasi berat. Dokter merencanakan resusitasi bolus cairan secara intravena atau intraoseus secepatnya. Jenis cairan apakah yang menjadi lini pertama untuk kondisi ini?",
                "options": ["Kristaloid", "Koloid", "Gelatin", "Starch", "Albumin 5%"],
                "correctAnswer": 0,
                "explanation": "Ringer Laktat atau Normal Saline (kristaloid) selalu menjadi lini pertama penanganan kegawatdaruratan syok hipovolemik (baik dehidrasi maupun perdarahan) pada populasi pediatrik maupun dewasa."
              },
              {
                "id": 16,
                "question": "Seorang pria berusia 25 tahun dibawa ke IGD setelah mengalami kecelakaan sepeda motor tanpa menggunakan helm. Saat diperiksa, pasien hanya membuka mata sesaat dan menepis tangan pemeriksa ketika diberikan rangsang nyeri di pangkal kuku, namun segera kembali menutup mata dan tertidur saat rangsang nyeri dilepaskan. Apakah kategori tingkat kesadaran pasien tersebut?",
                "options": ["Compos mentis", "Koma", "Sopor", "Somnolen", "Apatis"],
                "correctAnswer": 2,
                "explanation": "Kesadaran sopor/stupor ditandai dengan kemampuan pasien merespons hanya pada rangsangan nyeri yang kuat, namun kesadarannya langsung turun kembali bila stimulus dihentikan."
              },
              {
                "id": 17,
                "question": "Seorang perempuan 60 tahun datang dengan defisit neurologis fokal berupa hemiparesis dekstra onset 3 jam yang lalu. Pemeriksaan CT-scan kepala non-kontras menunjukkan area hipodensitas pada ganglia basalis kiri dengan batas yang mengabur (obscure boundaries) tanpa dijumpai midline shift. Apakah diagnosis radiologis pasien ini?",
                "options": ["Stroke iskemik akut", "Hematoma akut dan tumor maligna", "Hematoma kronis rekuren", "Perdarahan subaraknoid luas", "Perdarahan intraserebral akut"],
                "correctAnswer": 0,
                "explanation": "Area hipodens (gelap) dengan batas yang belum tegas/mengabur dan tanpa dorongan (midline shift) besar adalah gambaran klasik stroke iskemik non-hemoragik pada fase akut."
              },
              {
                "id": 18,
                "question": "Pasien laki-laki berusia 65 tahun tiba di ruang resusitasi IGD. Istri pasien melaporkan bahwa saat di jalan (sekitar 10 menit menuju RS), pasien mengalami kejang kelojotan selama 5 menit penuh tanpa sadar. Saat tiba, pasien terdengar snoring dan kesadaran masih menurun. Berdasarkan prinsip tatalaksana kegawatdaruratan neurologis, apakah tindakan prioritas yang harus dilakukan pertama kali?",
                "options": ["Pemeriksaan fisik dari kepala hingga kaki secara lengkap", "Mengamankan jalan napas dan menghentikan kejang jika masih berlangsung", "Mengambil sampel darah arteri untuk analisis gas darah", "Mendorong pasien langsung ke ruang CT-scan terdekat", "Memasang kateter vena sentral untuk pemberian akses cairan masif"],
                "correctAnswer": 1,
                "explanation": "Prioritas resusitasi (Primary Survey) ABC selalu mendahului diagnostik (seperti CT-scan atau darah lengkap). Mengamankan jalan napas (terutama karena ada snoring indikasi sumbatan jalan napas atas) adalah langkah paling awal."
              },
              {
                "id": 19,
                "question": "Seorang pasien glioblastoma multiforme telah menjalani reseksi pembedahan yang dilanjutkan dengan rangkaian radioterapi dan kemoterapi adjuvan. Dokter spesialis onkologi kemudian meminta pasien untuk kembali menjalani MRI kepala secara rutin pasca- terapi. Apakah tujuan utama dari pemeriksaan pencitraan pada fase ini?",
                "options": ["Memantau progresi lesi dan menilai respons jaringan terhadap terapi", "Mengukur tekanan perfusi serebral secara real-time", "Menegakkan diagnosis histopatologis definitif", "Memperbaiki midline shift yang masih tersisa", "Menentukan jenis obat kejang spesifik yang dapat dipakai"],
                "correctAnswer": 0,
                "explanation": "Post-terapi radiologis berfungsi utama untuk membandingkan ukuran massa tumor guna menentukan keberhasilan pengobatan atau melihat adanya kekambuhan (recurrence)."
              },
              {
                "id": 20,
                "question": "Saat mengedukasi keluarga pasien dengan epilepsi yang baru didiagnosis, dokter menjelaskan tentang berbagai tahapan kejang, salah satunya adalah fase di mana pasien sering kali tampak bingung, mengantuk, atau tidur lelap sesaat setelah hentakan motorik kejangnya berhenti. Apakah terminologi medis yang tepat untuk fase tersebut?",
                "options": ["Fase pemulihan menuju kesadaran normal yang terjadi pasca kejang", "Fase yang terjadi secara spesifik di sela-sela dua episode kejang elektrik", "Fase aura peringatan sebelum bangkitan kejang sesungguhnya terjadi", "Fase kejang subklinis tanpa manifestasi motorik tampak", "Fase relaksasi otot di tengah-tengah kejang tonik"],
                "correctAnswer": 0,
                "explanation": ""
              }
              ],
              "2022": [
              {
                "id": 1,
                "question": "Seorang pasien laki-laki berusia 35 tahun datang ke poliklinik dengan keluhan kejang fokal. Pasien mengeluhkan selalu merasakan adanya sensasi kesemutan dan kebas pada lengan kanannya selama beberapa detik sebelum kejang terjadi. Manakah lobus otak yang kemungkinan besar menjadi sumber bangkitan (fokus epileptogenik) pada pasien ini?",
                "options": ["Lobus parietal (Girus post-sentralis)", "Lobus frontal (Girus pre-sentralis)", "Lobus temporal", "Lobus oksipital", "Korteks insular"],
                "correctAnswer": 0,
                "explanation": "Sensasi kesemutan dan kebas adalah gejala sensorik somatik primer. Area yang memproses sensorik primer terletak di girus post-sentralis pada lobus parietal."
              },
              {
                "id": 2,
                "question": "Seorang laki-laki berusia 45 tahun akan menjalani operasi elektif dan memerlukan intubasi endotrakeal. Dokter anestesi memilih menggunakan propofol sebagai agen induksi anestesi. Apakah karakteristik farmakologis yang menjadi alasan utama pemilihan agen tersebut pada kasus ini?",
                "options": ["Memiliki onset kerja yang sangat cepat", "Dikontraindikasikan pada pasien dengan masalah jalan napas", "Memberikan efek analgesik yang sangat kuat", "Meningkatkan tekanan intrakranial secara signifikan", "Memiliki durasi kerja yang sangat panjang"],
                "correctAnswer": 0,
                "explanation": "Propofol adalah agen anestesi intravena yang sangat lipofilik, sehingga menembus sawar darah otak dengan cepat dan memberikan onset induksi yang sangat cepat (biasanya <1 menit)."
              },
              {
                "id": 3,
                "question": "Seorang laki-laki berusia 55 tahun dibawa ke IGD dalam keadaan tidak sadar dan tidak teraba nadi. Pemantauan EKG menunjukkan gambaran ventrikel fibrilasi (VF). Tim medis segera memberikan tindakan defibrilasi (shock). Apakah langkah selanjutnya yang paling tepat dilakukan?",
                "options": ["Memberikan kejut listrik ulang (shock again)", "Mengecek tanda vital dan meraba denyut nadi", "Memberikan injeksi adrenalin 1 mg IV", "Melanjutkan kompresi dada (RJP) segera", "Memberikan injeksi amiodaron 300 mg IV"],
                "correctAnswer": 3,
                "explanation": "Menurut algoritma Advanced Cardiovascular Life Support (ACLS) AHA, segera setelah pemberian defibrilasi (shock), RJP/kompresi dada harus langsung dilanjutkan selama 2 menit tanpa membuang waktu untuk mengecek nadi atau ritme."
              },
              {
                "id": 4,
                "question": "Seorang anak laki-laki berusia 2 tahun dibawa ke IGD karena mengalami kejang seluruh tubuh selama 3 menit di rumah. Saat tiba di IGD, anak tersebut masih dalam keadaan kejang. Suhu tubuh tercatat 39,5°C dan akses intravena belum terpasang. Apakah tatalaksana farmakologis awal yang paling direkomendasikan?",
                "options": ["Diazepam rektal", "Klonazepam rektal", "Fenitoin intravena", "Karbamazepin sirup oral", "Asam valproat sirup oral"],
                "correctAnswer": 0,
                "explanation": "Pada setting pra-rumah sakit atau IGD ketika jalur intravena (IV) belum terpasang, diazepam rektal adalah pilihan utama yang cepat dan aman untuk menghentikan kejang akut pada anak."
              },
              {
                "id": 5,
                "question": "Seorang perempuan berusia 60 tahun dibawa ke IGD dengan diagnosis stroke hemoragik. Pasien mengalami penurunan kesadaran yang dalam dan pola pernapasan tidak adekuat sehingga membutuhkan tindakan intubasi segera untuk mengamankan jalan napas (life- saving). Berapakah level Emergency Severity Index (ESI) untuk pasien tersebut?",
                "options": ["ESI 1", "ESI 2", "ESI 3", "ESI 4", "ESI 5"],
                "correctAnswer": 0,
                "explanation": "ESI level 1 dialokasikan untuk pasien yang membutuhkan intervensi penyelamatan nyawa segera (immediate life-saving intervention), seperti pasien yang memerlukan intubasi karena henti napas atau napas tidak adekuat."
              },
              {
                "id": 6,
                "question": "Seorang perempuan berusia 45 tahun datang dengan keluhan sakit kepala hebat, mual, muntah, demam, dan fotofobia. Hasil pemeriksaan cairan serebrospinal (LCS) menunjukkan peningkatan leukosit (leukositosis) dan penurunan kadar glukosa. Apakah pemeriksaan penunjang tambahan yang paling tepat untuk menentukan etiologi spesifik pada kasus ini?",
                "options": ["Pemeriksaan KOH", "Pemeriksaan Prokalsitonin serum", "Kultur cairan serebrospinal", "CT Scan kepala dengan kontras", "Pemeriksaan serologi VDRL"],
                "correctAnswer": 2,
                "explanation": "Temuan pleositosis (leukosit tinggi) dan hipoglikorakia (glukosa rendah) pada LCS sangat khas untuk meningitis bakterial. Kultur LCS adalah gold standard untuk mengetahui bakteri spesifik penyebabnya."
              },
              {
                "id": 7,
                "question": "Seorang pasien dengan kondisi immunocompromised didiagnosis mengalami Strongyloidiasis diseminata berdasarkan pemeriksaan laboratorium dan klinis. Dokter merencanakan pemberian terapi antelmintik eradikasi. Apakah pilihan farmakoterapi lini pertama untuk kondisi tersebut?",
                "options": ["Ivermectin", "Mebendazole", "Albendazole", "Pyrantel pamoate", "Praziquantel"],
                "correctAnswer": 0,
                "explanation": "Ivermectin adalah agen antelmintik pilihan pertama (drug of choice) untuk infeksi Strongyloides stercoralis, termasuk bentuk diseminatanya."
              },
              {
                "id": 8,
                "question": "Seorang pasien dicurigai mengalami tumor otak intraserebral. Dokter membutuhkan modalitas pencitraan radiologi yang memiliki superioritas dalam memberikan detail dan resolusi yang sangat tinggi pada jaringan lunak otak (soft tissue) dibandingkan dengan struktur tulang. Apakah modalitas radiologi yang paling tepat?",
                "options": ["Magnetic Resonance Imaging (MRI)", "Computed Tomography (CT) Scan", "Positron Emission Tomography (PET) Scan", "Ultrasonografi (USG) Transkranial", "Foto Polos Cranium (X-Ray)"],
                "correctAnswer": 0,
                "explanation": "MRI jauh lebih superior dibandingkan CT scan dalam menilai dan memberikan kontras resolusi yang sangat baik pada anatomi jaringan lunak (soft tissue) otak dan medula spinalis."
              },
              {
                "id": 9,
                "question": "Seorang bayi berusia 8 bulan dibawa ke IGD dengan diare profus dan tanda dehidrasi berat. Bayi tampak letargis, mata cekung, dan turgor kulit sangat lambat kembali. Dokter akan melakukan resusitasi cairan intravena secara bolus cepat. Apakah jenis cairan resusitasi yang paling tepat digunakan?",
                "options": ["Kristaloid", "Dextran", "Starch", "Gelatin", "Dekstrosa 5%"],
                "correctAnswer": 0,
                "explanation": "Cairan kristaloid isotonik (seperti Ringer Laktat atau NaCl 0.9%) adalah cairan lini pertama untuk resusitasi cairan pada kondisi dehidrasi berat atau syok hipovolemik pada anak dan bayi."
              },
              {
                "id": 10,
                "question": "Bayi perempuan berusia 6 bulan dibawa ke IGD karena mengalami kejang pada lengan dan tangan kanan. Ibunya menyatakan ini adalah kejang ketiga kalinya dengan pola yang sama pada hari ini. Suhu tubuh 40°C. Pemeriksaan fisik menunjukkan pasien kompos mentis dan tanda rangsang meningeal negatif. Apakah rencana pemeriksaan penunjang diagnostik yang paling komprehensif dan tepat diindikasikan?",
                "options": ["Darah rutin saja", "Darah rutin, pungsi lumbal, dan glukosa darah", "Darah rutin, pungsi lumbal, glukosa darah, elektrolit, dan urinalisis", "Darah rutin, pungsi lumbal, glukosa darah, elektrolit, urinalisis, dan CT scan kepala", "Pemeriksaan EEG dan MRI Kepala"],
                "correctAnswer": 2,
                "explanation": "Kejang fokal berulang pada bayi <1 tahun dengan demam tinggi mewajibkan pemeriksaan untuk menyingkirkan meningitis (Pungsi Lumbal) dan ketidakseimbangan metabolik (Glukosa, Elektrolit, Urinalisis). CT Scan kepala umumnya tidak diindikasikan secara rutin kecuali ada tanda peningkatan TIK yang jelas."
              },
              {
                "id": 11,
                "question": "Seorang anak balita dievaluasi kesadarannya di IGD setelah terjatuh. Pada pemeriksaan Pediatric Glasgow Coma Scale (PGCS), didapatkan respons motorik menghindar hanya saat diberikan stimulus nyeri, mata membuka secara spontan tanpa perlu ditepuk/dipanggil, dan secara verbal anak terus menangis serta tampak iritabel. Berapakah skor PGCS pasien tersebut?",
                "options": ["10", "11", "12", "13", "14"],
                "correctAnswer": 2,
                "explanation": "Eye: Terbuka spontan (Skor 4). Verbal: Menangis/iritabel (Skor 4). Motor: Menghindar terhadap nyeri/withdrawal (Skor 4). Total = 12."
              },
              {
                "id": 12,
                "question": "Seorang pemuda berusia 25 tahun menjalani perekaman EKG. Ditemukan denyut jantung rata-rata 60 kali/menit dengan gambaran interval R-R yang bervariasi secara reguler (ireguler secara periodik). Diketahui bahwa variasi denyut jantung ini berkaitan erat dengan fase respirasi pasien tanpa adanya blok atrioventrikular. Apakah mekanisme fisiologis yang mendasari variasi tersebut?",
                "options": ["Denyut jantung menurun saat inspirasi dan meningkat saat ekspirasi", "Ireguleritas murni terjadi karena pergerakan artefak otot pernapasan", "Denyut jantung meningkat saat inspirasi dan menurun saat ekspirasi", "Terjadi hambatan impuls yang progresif pada nodus atrioventrikular", "Terdapat hambatan aliran listrik pada cabang berkas His kanan"],
                "correctAnswer": 2,
                "explanation": "Kasus ini menjelaskan Sinus Arrhythmia pernapasan yang fisiologis. Saat inspirasi, tonus vagal menurun sehingga HR meningkat; saat ekspirasi, tonus vagal meningkat sehingga HR menurun."
              },
              {
                "id": 13,
                "question": "Seorang laki-laki berusia 56 tahun tiba-tiba mengalami kelemahan pada tungkai bawah (paraparesis inferior), gangguan berjalan (gait apraxia), serta inkontinensia urin. Tidak didapatkan kelemahan yang signifikan pada lengan atas. Berdasarkan presentasi klinis tersebut, area vaskularisasi arteri manakah yang paling mungkin mengalami infark/oklusi?",
                "options": ["Arteri serebri interna", "Arteri serebri anterior", "Arteri serebri media", "Arteri serebri posterior", "Arteri vertebrobasilar"],
                "correctAnswer": 1,
                "explanation": "Infark pada teritori Anterior Cerebral Artery (ACA) khas memberikan gejala kelemahan motorik dan sensorik kontralateral yang jauh lebih berat pada tungkai (ekstremitas bawah) dibandingkan lengan, disertai kelainan perilaku, apraksia berjalan, dan inkontinensia."
              },
              {
                "id": 14,
                "question": "Pada pasien pasca-tiroidektomi total, dokter melakukan pengetukan pada nervus fasialis di anterior telinga dan mengamati adanya kedutan otot wajah spontan (Chvostek's sign positif). Temuan ini sangat mengarah pada kondisi hipokalsemia. Apakah tanda klinis spesifik lainnya yang dapat diinduksi untuk mengkonfirmasi kondisi elektrolit tersebut?",
                "options": ["Trousseau's sign", "Kernig's sign", "Brudzinski's sign", "Babinski's sign", "Hoffman's sign"],
                "correctAnswer": 0,
                "explanation": "Trousseau\'s sign dievaluasi dengan memompa manset sfigmomanometer di atas tekanan sistolik selama 3 menit, yang akan memicu spasme karpopedal pada pasien hipokalsemia."
              },
              {
                "id": 15,
                "question": "Seorang pasien dengan riwayat Penyakit Ginjal Kronik (CKD) stadium akhir datang dengan letargi, disorientasi, dan mioklonus. Pasien didiagnosis dengan ensefalopati uremikum. Jika dilakukan pemeriksaan Elektroensefalografi (EEG), apakah temuan awal yang paling sering dijumpai pada ensefalopati metabolik ini?",
                "options": ["Kemunculan gelombang lambat delta secara persisten", "Perlambatan pada frekuensi gelombang alfa dominan di posterior", "Kemunculan gelombang lambat theta secara difus", "Gelombang alfa intermiten pada area frontal", "Gambaran kompleks spike-and-wave 3 Hz"],
                "correctAnswer": 1,
                "explanation": "Pada stadium awal ensefalopati uremikum/metabolik, kelainan EEG yang paling dini terlihat adalah disorganisasi dan perlambatan (slowing) gelombang latar belakang (alfa) di area posterior."
              },
              {
                "id": 16,
                "question": "Seorang pasien laki-laki berusia 55 tahun tiba di IGD dengan onset hemiparesis kanan mendadak sejak 1 jam yang lalu. CT Scan kepala non-kontras menyingkirkan adanya perdarahan intrakranial. Dokter merencanakan pemberian terapi rekanalisasi. Apakah agen farmakologis yang memiliki efek langsung untuk melisiskan gumpalan trombus pada kasus stroke iskemik akut?",
                "options": ["Aspirin", "Clopidogrel", "Alteplase", "Heparin", "Citicoline"],
                "correctAnswer": 2,
                "explanation": "Recombinant tissue plasminogen activator (rt-PA) seperti Alteplase adalah agen trombolitik yang bekerja langsung memecah/melisiskan trombus. Obat lain seperti aspirin atau clopidogrel hanya mencegah agregasi platelet lanjutan."
              },
              {
                "id": 17,
                "question": "Seorang bayi baru lahir menunjukkan fitur wajah dismorfik, ensefalopati progresif, letargi, dan kejang yang sulit diatasi. Dokter anak mencurigai bayi tersebut menderita Inborn Errors of Metabolism (IEM) yang muncul sejak lahir. Apakah patofisiologi utama yang sering menyebabkan kerusakan otak pada kelainan metabolik tersebut?",
                "options": ["Disrupsi pelepasan neurotransmiter di celah sinaps", "Cedera sitotoksik akibat penumpukan metabolit toksik", "Inflamasi autoimun yang menargetkan mielin", "Infeksi oportunistik pada cairan serebrospinal", "Hiperperfusi serebral sekunder akibat asidosis"],
                "correctAnswer": 1,
                "explanation": "Pada kelainan bawaan metabolisme (IEM), ketiadaan atau defisiensi enzim tertentu menyebabkan terhentinya jalur metabolik. Hal ini memicu akumulasi prekursor atau substrat yang bersifat toksik bagi sel otak (seperti amonia atau asam organik), memicu kerusakan sitotoksik langsung"
              }
              ],
              "2023": [
              {
                "id": 1,
                "question": "Seorang bayi perempuan berusia 6 bulan dibawa ke IGD karena mengalami kejang pada lengan dan tangan kanannya. Suhu tubuh pasien tercatat 40°C. Berdasarkan alloanamnesis, ini adalah ketiga kalinya kejang terjadi dengan gejala yang serupa. Kejang berlangsung singkat, yaitu hanya sekitar 2 menit. Apakah jenis kejang yang paling tepat untuk mendeskripsikan kondisi pasien tersebut?",
                "options": ["Meningitis", "Kejang demam kompleks", "Status epileptikus", "Kejang demam sederhana", "Epilepsi fokal"],
                "correctAnswer": 3,
                "explanation": "Sesuai dengan materi (meskipun gejalanya fokal, kunci pada rekapan mahasiswa mencatat ini sebagai Simple Febrile Seizure berdasarkan durasi < 15 menit dan suhu tinggi).*"
              },
              {
                "id": 2,
                "question": "Seorang pasien laki-laki berusia 45 tahun dicurigai mengalami infeksi sistem saraf pusat. Dokter menginstruksikan prosedur pungsi lumbal untuk mengambil spesimen cairan serebrospinal (CSF). Cairan tersebut ditampung ke dalam beberapa tabung terpisah sesuai protokol laboratorium. Apakah parameter utama yang diperiksa pada tabung pertama dari spesimen tersebut?",
                "options": ["Protein total", "Hitung jenis leukosit (Differential Count)", "Hitung sel (Cell Count)", "Kultur bakteri", "Pewarnaan Gram"],
                "correctAnswer": 0,
                "explanation": "Tabung 1 pada pungsi lumbal umumnya dialokasikan untuk pemeriksaan kimiawi (protein dan glukosa). Tabung 2 untuk mikrobiologi, dan tabung 3 untuk sel/hematologi."
              },
              {
                "id": 3,
                "question": "Seorang wanita berusia 30 tahun datang ke poliklinik kandungan untuk konsultasi prakonsepsi. Ia memiliki riwayat melahirkan anak pertama dengan defek tabung saraf (spina bifida) dan saat ini sedang merencanakan kehamilan anak kedua. Dokter menyarankan suplementasi nutrisi spesifik untuk mencegah berulangnya kelainan kongenital tersebut. Apakah suplemen yang paling tepat diberikan pada pasien ini?",
                "options": ["Asam valproat", "Asam folat", "Tiamin", "Vitamin B12", "Kalsium"],
                "correctAnswer": 1,
                "explanation": "Suplementasi folic acid perikonsepsional terbukti menurunkan insidensi Neural Tube Defects (NTD) setidaknya hingga 50%."
              },
              {
                "id": 4,
                "question": "Seorang pasien pria berusia 55 tahun menjalani pemeriksaan laboratorium rutin. Hasil panel metabolik menunjukkan kadar natrium serum yang rendah, namun pemeriksaan osmolalitas serum dalam batas normal. Pasien sama sekali tidak menunjukkan gejala neurologis (asimtomatik). Dokter mendiagnosis pasien dengan hiponatremia isotonik (pseudohiponatremia). Apakah kondisi yang paling mungkin mendasari fenomena tersebut?",
                "options": ["Hipoglikemia", "Hiperlipidemia", "Hiperkalemia", "Penyalahgunaan diuretik", "Hiperkalsemia"],
                "correctAnswer": 1,
                "explanation": "Pseudohiponatremia (hiponatremia palsu/isotonik) sering kali disebabkan oleh hiperlipidemia atau hiperproteinemia berat yang menggantikan ruang air dalam serum."
              },
              {
                "id": 5,
                "question": "Seorang anak laki-laki dengan riwayat penyakit ginjal kronis stadium akhir dibawa ke IGD dengan penurunan kesadaran. Pasien diketahui melewatkan jadwal rutin hemodialisisnya. Pemeriksaan laboratorium menunjukkan adanya perburukan penumpukan ureum di dalam darah serta ketidakseimbangan elektrolit yang masif. Apakah komplikasi neurologis yang paling mungkin terjadi pada pasien ini?",
                "options": ["Ensefalopati uremikum", "Hipoglikemia", "Hipokalemia", "Ensefalopati hepatikum", "Ensefalopati hipoksik-iskemik"],
                "correctAnswer": 0,
                "explanation": "Pasien gagal ginjal dengan penumpukan ureum parah yang melewatkan dialisis berisiko tinggi mengalami uremic encephalopathy."
              },
              {
                "id": 6,
                "question": "Seorang anak berusia 2 tahun dibawa ke IGD dalam keadaan kejang aktif. Berdasarkan anamnesis dari orang tua, anak tersebut sama sekali tidak mengalami demam sebelum atau saat kejang terjadi. Saat ini akses intravena belum terpasang pada pasien. Apakah obat lini pertama yang paling tepat untuk diberikan?",
                "options": ["Fenobarbital", "Fenitoin", "Klonazepam", "Karbamazepin", "Asam valproat"],
                "correctAnswer": 2,
                "explanation": "Lini pertama untuk tatalaksana kejang adalah golongan Benzodiazepine (seperti Klonazepam, Midazolam, Lorazepam, Diazepam)."
              },
              {
                "id": 7,
                "question": "Seorang bayi baru lahir prematur dirawat di NICU karena mengalami asfiksia perinatal. Walaupun saat ini tingkat kesadarannya masih compos mentis, bayi tersebut memiliki risiko tinggi mengalami kerusakan seluler global akut akibat berkurangnya suplai oksigen dan aliran darah ke otak. Apakah diagnosis yang paling tepat untuk komplikasi neurologis yang diantisipasi pada kasus ini?",
                "options": ["Ensefalopati hipoksik-iskemik", "Ensefalopati uremikum", "Ensefalopati hepatikum", "Ensefalopati hipertensif", "Ensefalopati metabolik"],
                "correctAnswer": 0,
                "explanation": "Asfiksia perinatal sangat berkaitan erat dengan perkembangan Hypoxic-Ischemic Encephalopathy (HIE) pada neonatus."
              },
              {
                "id": 8,
                "question": "Seorang pasien dengan riwayat epilepsi diresepkan obat antikonvulsan baru oleh dokter spesialis saraf. Dokter menjelaskan bahwa obat tersebut bekerja dengan cara menghambat enzim GABA transaminase secara ireversibel, sehingga kadar inhibitor neurotransmiter GABA di otak meningkat. Apakah nama obat yang diberikan pada pasien tersebut?",
                "options": ["Gabapentin", "Lamotrigin", "Levetirasetam", "Vigabatrin", "Topiramat"],
                "correctAnswer": 3,
                "explanation": "Vigabatrin (-vinyl-GABA) adalah inhibitor GABA transaminase ireversibel."
              },
              {
                "id": 9,
                "question": "Seorang pasien pascastroke mengalami gangguan komunikasi. Pasien tampak kesulitan dan membutuhkan usaha keras untuk memproduksi kata-kata secara verbal, namun pemahamannya terhadap perintah atau percakapan masih relatif baik. Dokter mendiagnosis pasien dengan Afasia Broca. Di manakah letak lesi anatomis yang paling mungkin mendasari kondisi tersebut?",
                "options": ["Girus frontalis inferior", "Girus temporalis superior", "Girus temporalis transversus", "Girus presentralis", "Girus postsentralis"],
                "correctAnswer": 0,
                "explanation": "Area Broca (motorik untuk bicara) terletak di inferior frontal gyrus (pars triangularis / opercularis)."
              },
              {
                "id": 10,
                "question": "Seorang wanita berusia 40 tahun datang ke klinik dengan keluhan sakit kepala yang terus-menerus. Ia mengaku sering mengonsumsi obat pereda nyeri yang dibeli bebas (OTC) selama 2 tahun terakhir yang awalnya meredakan gejala. Namun, kini sakit kepalanya menjadi jauh lebih hebat, disertai muntah dan penurunan fungsi kognitif. Berdasarkan pola perjalanan penyakitnya, apakah klasifikasi diagnosis yang paling tepat?",
                "options": ["Eksaserbasi akut kronis", "Kronik progresif", "Subakut progresif", "Akut progresif", "Kronik non-progresif"],
                "correctAnswer": 1,
                "explanation": "Nyeri kepala akibat penggunaan analgetik berlebih (MOH) yang berlangsung tahunan dan memburuk dengan defisit kognitif digolongkan dalam pola kronik progresif."
              },
              {
                "id": 11,
                "question": "Seorang pria berusia 35 tahun dibawa ke IGD pascakecelakaan lalu lintas. Saat dinilai tingkat kesadarannya, pasien hanya membuka mata ketika namanya dipanggil keras. Saat ditanya keluhannya, pasien memberikan jawaban yang tidak sesuai (inappropriate words). Pada penilaian motorik, pasien menggerakkan tangan untuk melokalisasi nyeri saat diberikan rangsangan sternal, namun tidak dapat mengikuti perintah verbal. Berapakah skor Glasgow Coma Scale (GCS) pada pasien ini?",
                "options": ["E3V3M5", "E3V2M4", "E2V2M3", "E2V2M5", "E2V3M4"],
                "correctAnswer": 0,
                "explanation": "Buka mata saat dipanggil = E3; Menjawab tidak sesuai (inappropriate words) = V3; Melokalisasi nyeri tanpa komando = M5."
              },
              {
                "id": 12,
                "question": "Seorang pria berusia 50 tahun dibawa oleh keluarganya ke IGD karena mengalami kejang. Menurut keterangan keluarga, kejang bermula dengan tangan kanannya yang mendadak menjadi sangat kaku. Selama serangan tersebut berlangsung, pasien juga mengalami kehilangan kesadaran penuh. Berdasarkan presentasi klinis tersebut, apakah tipe kejang yang dialami pasien?",
                "options": ["Kejang umum tonik-klonik", "Kejang umum tonik", "Kejang umum klonik", "Kejang fokal tonik", "Kejang fokal klonik"],
                "correctAnswer": 3,
                "explanation": "Kejang bermula pada satu sisi anggota gerak (fokal) dan menimbulkan kekakuan (tonik)."
              },
              {
                "id": 13,
                "question": "Seorang dokter spesialis radiologi sedang mengevaluasi gambar otak dari seorang pasien yang dicurigai mengalami stroke iskemik hiperakut. Gambar tersebut menampilkan resolusi jaringan lunak yang sangat baik dengan berbagai sekuens pembobotan seperti T1, T2, dan DWI (Diffusion Weighted Imaging). Apakah modalitas pencitraan yang sedang dievaluasi oleh dokter tersebut?",
                "options": ["MRI", "CT Scan", "USG", "PET Scan", "X-Ray"],
                "correctAnswer": 0,
                "explanation": "Modalitas yang mengandalkan sekuens seperti T1, T2, FLAIR, DWI, dan ADC adalah Magnetic Resonance Imaging (MRI)."
              },
              {
                "id": 14,
                "question": "Seorang pria berusia 60 tahun dengan riwayat hipertensi datang ke IGD dengan kelemahan separuh tubuh mendadak. Pada pemeriksaan CT scan kepala tanpa kontras (NCCT), ditemukan lesi hiperdens yang dikelilingi edema. Selanjutnya, pada MRI sekuens T2 FLAIR, terlihat area yang terang (bright) di lobus parietal. Apakah kemungkinan temuan patologis utama pada pasien ini?",
                "options": ["Perdarahan intraserebral", "Perdarahan subdural", "Angiopati hipertensif", "Malformasi arteriovenosa", "Angiopati amiloid"],
                "correctAnswer": 0,
                "explanation": "Gambaran CT hiperdens (darah) yang dikelilingi area hipodens (edema perifokal) mengindikasikan Intracerebral Hemorrhage (ICH)."
              },
              {
                "id": 15,
                "question": "Seorang pasien datang ke IGD dengan kebingungan mental akut. Tanda vital menunjukkan tekanan darah 210/110 mmHg. Hasil laboratorium menunjukkan glukosa darah 100 mg/dL, leukosit 9.000/µL, dan hipoalbuminemia berat akibat sindrom nefrotik. Dokter mencurigai adanya ekstravasasi cairan akibat penurunan tekanan onkotik plasma dan permeabilitas vaskular yang tinggi akibat kondisi tekanan darah. Apakah diagnosis komplikasi yang paling mungkin?",
                "options": ["Ensefalopati hipertensif", "Ensefalopati uremikum", "Ensefalopati metabolik", "Ensefalopati hepatikum", "Ensefalopati hipoksik-iskemik"],
                "correctAnswer": 0,
                "explanation": "Pasien sindrom nefrotik dengan hipoalbuminemia memiliki penurunan tekanan onkotik, yang bersamaan dengan lonjakan tekanan darah (210/110), sangat rentan menjadi ensefalopati hipertensif."
              },
              {
                "id": 16,
                "question": "Seorang anak laki-laki berusia 12 tahun dibawa ke IGD karena sakit kepala hebat yang timbul mendadak disertai muntah sebanyak 4 kali. Tanda vital: TD 140/80 mmHg, HR 98 x/menit, RR 22 x/menit. Hasil pencitraan kepala menunjukkan adanya lesi hipodens yang disertai perdarahan dengan estimasi volume 25 cc. Apakah kemungkinan penyebab kondisi pada pasien tersebut?",
                "options": ["Aneurisma serebral", "Perdarahan intratumoral", "Angiopati amiloid", "Malformasi arteriovenosa", "Infark serebral luas"],
                "correctAnswer": 1,
                "explanation": "Lesi hipodens pada CT scan dapat mengindikasikan edema atau jaringan tumor, yang bila disertai perdarahan volume terukur sering dikaitkan dengan intratumoral hemorrhage"
              }
              ]
            }
          }
        }
      },
      "cumex-2": {
        name: "Cumex 2",
        types: {
          "reguler": {
            name: "Reguler",
            locked: true,
            years: { "2021": [], "2022": [], "2023": [] }
          },
          "iup": {
            name: "IUP",
            locked: true,
            years: { "2021": [], "2022": [], "2023": [] }
          }
        }
      },
      "uas": {
        name: "UAS",
        types: {
          "reguler": {
            name: "Reguler",
            locked: true,
            years: { "2021": [], "2022": [], "2023": [] }
          },
          "iup": {
            name: "IUP",
            locked: true,
            years: { "2021": [], "2022": [], "2023": [] }
          }
        }
      }
    }
  }
};
